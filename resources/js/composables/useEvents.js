/**
 * useEvents — mock data composable
 * In a real Laravel 13 / Vue 3 app, these would be fetched via
 * fetch('/api/events') or an Inertia.js prop from the backend.
 */
...
export function useEvents() {
  const nextEvents = BASE_EVENTS.filter(e => e.tags.includes('next'))
  const popularEvents = BASE_EVENTS.filter(e => e.tags.includes('popular'))
  const featuredEvents = BASE_EVENTS.filter(e => e.tags.includes('featured'))

  return { nextEvents, popularEvents, featuredEvents }
}
